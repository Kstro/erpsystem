 ...
 */
class CrmMeetingsController extends Controller
{
    /**
     * Lists all CrmActividad entities.
     *
     * @Route("/", name="admin_meetings_index")
     * @Method("GET")
     */
    public function indexAction()
    {
        ...
        try{
            ...
            return $this->render('crmactividad/index_meetings.html.twig', array(
     ...
     *
     * @Route("/activities-meetings/data/list", name="admin_activities_meetings_data")
     */
    public function dataactivitiesAction(Request $request)
    {
            try {
                ...
                $sql = "SELECT obj.id as id FROM ERPCRMBundle:CrmActividad obj "
                            ."JOIN obj.tipoActividad tact WHERE tact.id=3";
...
//                        break;
    ...
    public function saveajaxAction(Request $request)
    {
        ...
        if($isAjax){
            try {
                ...
                // $tipoActividadObj = $em->getRepository('ERPCRMBundle:CrmTipoActividad')->find($tipoTasks);//Task o tarea
                $tipoActividadObj = $em->getRepository('ERPCRMBundle:CrmTipoActividad')->find(3);//Meetings
     ...
     * Retrieve calls
     *
     * @Route("/meetings/retrieve", name="admin_meetings_retrieve_ajax",  options={"expose"=true}))
     * @Method("POST")
     */
    public function retrieveajaxAction(Request $request)
    {
        try {
            ...
            if(count($crmActividadObj)!=0){
                ...
                $data['location']=$crmActividadObj->getDireccion();
                // var_dump($data);
                // die();
