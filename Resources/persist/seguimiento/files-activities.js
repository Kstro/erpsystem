$(document).ready(function() {
	...
	/*/////Fin borrar files*/
        ...
        $('#frmFiles').on('submit',(function(event) {
            ...
            var idAct= $('#txtId').val();
            formData.append('param1',idAct);
            $.ajax({
                url: Routing.generate('admin_files_add_activities_ajax'),
                type: "POST",            
                data: formData,
                contentType: false,    
                cache: false,           
                ...
                success: function(data)  
                {
                    if (data.error) {
                        ...
                        var item='';
                        var addItem='<div class="col-xs-1" style="vertical-align:middle;"><a id="'+data.idFileg+'" href="/files/activities/'+data.nombreFile+'" class="fileDelete"><i style="margin-top:3px;vertical-align:middle;" class="fa fa-remove"></i></a></div><div class="col-xs-10"><a target="_blank" href="../../../files/activities/'+data.nombreFile+'">'+data.nombreFile+'</a></div>';
...
function deleteFiles(idDom){
	...
	$.ajax({
		url: Routing.generate('admin_files_delete_activities_ajax'),
		type: 'POST',
