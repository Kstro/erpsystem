$(document).ready(function() {
	...
	$('#frmCase').on('submit',(function(event) {
		...
		if (errores==0) {
				...
				$.ajax({
					url: Routing.generate('admin_types_case_save_ajax'),
					type: "POST",
	...
	$(document).on('click', '#caseTipeList>tbody>tr>td:nth-child(2)', function(event) {
		/*/////Definición de variables*/
		...
		if (text=='TD' && id!=idForm && selected==0) {
			...
			$.ajax({
				url: Routing.generate('admin_tipo_case_retrieve_ajax'),
				type: 'POST',
				data: {param1: id},
				success:function(data){
					if(data.error){
	...
	$(document).on('click', '.btnDelete', function(event) {
		...
		swal({
                        ...
                        if (isConfirm) {
                            	$.ajax({
					url: Routing.generate('admin_tipo_case_delete_ajax'),
					type: 'POST',
					data: {param1: ids},
					success:function(data){
						if(data.error){
