$(document).ready(function() {
	...
	$(document).on('click', '#btnSave', function(event) {
		...
		var $btn = $(this).button('loading');
    		var id=$('#txtId');
		var name=$('#txtName');
		var table = $('#satisfactionList').DataTable();
		var errores = 0;
/*		//Contador de errores, para antes de la persistencia*/
		...
		if (errores==0) {
			$.ajax({
				url: Routing.generate('admin_customer_satisfaction_save_ajax'),
				type: 'POST',
	...
	$(document).on('click', '#satisfactionList>tbody>tr>td:nth-child(2)', function(event) {
		/*/////Definición de variables*/
		...
		if (text=='TD' && id!=idForm && selected==0) {
			$.ajax({
				url: Routing.generate('admin_customer_satisfaction_retrieve_ajax'),
				type: 'POST',
				data: {param1: id},
				success:function(data){
					if(data.error){
	...
	$(document).on('click', '.btnDelete', function(event) {
		...
		swal({
                        ...
                        if (isConfirm) {
                            	$.ajax({
					url: Routing.generate('admin_customer_satisfaction_delete_ajax'),
					type: 'POST',
					data: {param1: ids},
					success:function(data){
						if(data.error){
